...
import com.example.negocio.routeContas
...
@Location("/contas/{contaText}") class RouteContas_contaText(val contaText: String)
@KtorExperimentalLocationsAPI
@Location("/contas") class RouteContas
...
fun Application.module(testing: Boolean = false) {
    ...
    val contasConfig = environment.config.config("contas")
...
//    val sessionkey = hex(key)
    ...
    val uploadDirPath: String = contasConfig.property("upload.dir").getString()
    val uploadDir = File(uploadDirPath)
    if (!uploadDir.mkdirs() && !uploadDir.exists()) {
        throw IOException("Failed to create directory ${uploadDir.absolutePath}")
    }

    val cacheDirPath: String = contasConfig.property("cache.dir").getString()
